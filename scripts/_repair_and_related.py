...
for path in sorted(glob.glob("src/pages/servicos/*/index.astro")):
    ...
    # Remove duplicate related blocks if any
    while text.count('class=\\"serv-inc__related\\"') > 1:
        text = re.sub(
            r'<div class=\\"serv-inc__related\\">.*?</div>\s*\\n\s*(?=<div class=\\"serv-inc__related\\">)',
            "",
            text,
            count=1,
            flags=re.DOTALL,
    ...
    with open(path, "w", encoding="utf-8", newline="") as f:
        f.write(updated)
    cards = updated.count("card-dr-pattern")
    print(f"ok {slug}: card-dr-pattern={cards}")
